...
func main() {
	...
	// config logging
	fp := filepath.Join(".", "serverLog.log")
	...
	fmt.Println(runprovide(ip, tcpport, quicport, maxqwlepp, filter, psi))
}

func runprovide(ip string, tcpport int, quicport int, maxqwlepp uint, filter bool, psi bool) error {
	...
	bsopt = append(bsopt, bitswap.MaxQueuedWantlistEntriesPerPeer(maxqwlepp))
